                       ...
                       <form action="/analysis/list" method="post">
                        <div class="row">
                            ...
                            <div class="col-sm-2">
                                <select name="type" class="selectpicker form-control">
                                    ...
                                    <option  <c:if test="${type==1}">selected</c:if> value="1">项目标题</option>
                            ...
                            <div class="col-sm-2 text-right">
                            		<a href="/save-analysis.jsp" class="btn btn-sm btn-primary" type="button"><i class="fa fa-plus-circle"></i> 添加需求</a>
                            	</div>
                        </div>
                       </form>
                            ...
                            <table class="table table-striped list-table">
                                ...
                                <c:forEach items="${list}" var="analysis">
                                    <tr>
                                        ...
                                        <td>
	                                        	<a href="/analysis/show?id=${analysis.id}"><i class="glyphicon glyphicon-eye-open  text-navy"></i></a>
	                                        	<a href="/analysis/toUpdate?id=${analysis.id}"><i class="glyphicon glyphicon-edit  text-navy"></i></a>
                        ...
                        <div class="row">
							...
							<div class="col-sm-7 text-right">
                                ...
                                <a href="/analysis/list?pageNum=${page.pageNum-1}&type=${type}&keyword=${keyword}" type="button" class="btn btn-sm btn-white <c:if test="${page.pageNum-1<1}">disabled</c:if> "><i class="glyphicon glyphicon-chevron-left"></i>
                                </a>
                                <c:forEach begin="1" end="${page.pages}" varStatus="vs">

                                    <a href="/analysis/list?pageNum=${vs.index}&type=${type}&keyword=${keyword}" class="btn btn-sm btn-white    <c:if test="${page.pageNum==vs.index}">active</c:if> ">${vs.index}</a>
                                </c:forEach>

                                <a href="/analysis/list?pageNum=${page.pageNum+1}&type=${type}&keyword=${keyword}" type="button" class="btn btn-sm btn-white <c:if test="${page.pageNum+1>page.pages}">disabled</c:if>"><i class="glyphicon glyphicon-chevron-right"></i>
                                </a>
                            </div>
   ...
   <script>
	$(document).ready(function() {
        ...
        function doRemove(ids){
            $.post("/analysis/remove","ids="+ids,function (result) {
